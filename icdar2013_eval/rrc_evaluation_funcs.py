...
def load_zip_file(file, fileNameRegExp='', allEntries=False):
    ...
    for name in archive.namelist():
        ...
        if addFile:
            pairs.append([keyName, archive.read(name).decode('utf-8')])
        else:
            if allEntries:
...
def validate_lines_in_file(fileName, contents, CRLF=True, LTRB=True, withTranscription=False, withConfidence=False,
...
def main_evaluation(p,
    ...
    gt, subm = validate_data_fn(p['g'], p['s'], evalParams)
    ...
    if show_result:
        print("Calculated! {}".format(json.dumps(resDict['method'])))
