...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    res_files = glob.glob(os.path.join(submFilePath, "*.txt"))
    regexp = evaluationParams['DET_SAMPLE_NAME_2_ID']
    subm = []
    for f in res_files:
        addFile = True
        baseName = os.path.basename(f)
        keyName = baseName
        m = re.match(regexp, baseName)
        if m == None:
            addFile = False
        else:
            if len(m.groups()) > 0:
                keyName = m.group(1)
        if addFile:
            with open(f, "rt") as of:
                content = of.read()
            subm.append([keyName, content])

    subm = dict(subm)

    # Validate format of GroundTruth
    for k in gt:
        points, _, transcriptions = rrc_evaluation_funcs.get_tl_line_values_from_file_contents(gt[k],
                                                                                               evaluationParams['CRLF'],
                                                                                               True, True, False)
        gt[k] = (points, transcriptions)

    # Validate format of results
    for k in subm.keys():
        if (k in gt) == False:
            raise Exception("The sample %s not present in GT" % k)
        subm[k], _, _ = rrc_evaluation_funcs.get_tl_line_values_from_file_contents(subm[k],
                                                                                   evaluationParams['CRLF'],
                                                                                   True, False, False)
...
def evaluate_method(gt, subm, evaluationParams):
    ...
    for gtKey in gt.keys():
        ...
        pointsList, transcriptionsList = gt[gtKey]
...
def eval_result(gt_file, res_path):
    p = {'g': gt_file, 's': res_path}
    return rrc_evaluation_funcs.main_evaluation(p, default_evaluation_params, validate_data, evaluate_method)


if __name__ == '__main__':
    res = eval_result("gt.zip", "/tmp/res")['method']
    print(res)
    print("{}, {}, {}".format(res['hmean'], res['recall'], res['precision']))
